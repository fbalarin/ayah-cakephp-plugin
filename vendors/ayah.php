 ...
 * 
 */
if (file_exists(dirname(dirname(__FILE__)) . DS .'config' .DS.'ayah_config.php')) {
	require_once(dirname(dirname(__FILE__)) . DS .'config' .DS.'ayah_config.php');
} else {
	die("AYAH: ayah_config.php missing; will require setting of specific values.");
}
...
class AYAH {
	...
	public function getPublisherHTML() {
        ...
        public function getPublisherScript() {
		$url = 'https://' . $this->ayah_web_service_host . "/ws/script/" . 
			urlencode($this->ayah_publisher_key);
			
		return $url;
	}
	public function getPublisherIframe() {
		
		$response = $this->doHttpsPostReturnJSONArray($this->ayah_web_service_host,"/ws/getIframe/", array('publisher_key' => $this->ayah_publisher_key));
	...
	public function __construct($params = array()) {
		...
		if (defined('AYAH_WEB_SERVICE_HOST')) {
			$this->ayah_web_service_host = AYAH_WEB_SERVICE_HOST;
                ...
                if (defined('AYAH_TIMEOUT')) {
			$this->ayah_timeout = AYAH_TIMEOUT;
		}
		// Lastly grab the parameters input and save them
		foreach (array_keys($params) as $key) {
			if (in_array($key, array("publisher_key", "scoring_key", "web_service_host", "timeout"))) {
				$variable = "ayah_" . $key;
		...
		if ($this->ayah_web_service_host == "") {
			error_log("AYAH::__construct: Warning: Web service host is not defined.  This won't work.");
                ...
                if ($this->ayah_timeout == "") {
			error_log("AYAH::__construct: Warning: Web service timeout is not defined.  This won't work.");
		}
